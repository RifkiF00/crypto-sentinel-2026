...
// NOTE: All hardcoded SCENARIOS removed. GNN Workbench now uses ONLY real data
// from the backend API (/api/v1/sentinel/gnn/live-subgraph/{account_id}) which
// queries transaction_logs or PaySim dataset with SHAP-based XAI explanations.
...
export default function GNNVisualization({ addToast, onOpenCustomer360, onCreateCase, selectedEntity, streamingTransactions = [], isStreaming = false, liveNodes: liveNodesProp, setLiveNodes: setLiveNodesProp, liveEdges: liveEdgesProp, setLiveEdges: setLiveEdgesProp, lastProcessedIndex: lastProcessedIndexProp, setLastProcessedIndex: setLastProcessedIndexProp, detectedPatterns: detectedPatternsProp, setDetectedPatterns: setDetectedPatternsProp, edgeTimersRef: edgeTimersRefProp }) {
  ...
  // Scenario State — NO hardcoded mock data. All data comes from backend API.
  // The workbench starts in standby mode. A graph loads ONLY when the investigator
  // navigates here from an alert/transaction (selectedEntity is set).
  const hasActiveInvestigation = Boolean(selectedEntity);
  // scenario is always null now — liveScenario from API is the sole data source
  const scenario = null;
  ...
  }, [selectedEntity, activeScenario, nodePositions, zoom]);
    ...
    <div className="gnn-professional-monitor" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div className="card" style={{ padding: '16px 20px', background: 'var(--bg-card)', border: '1px solid var(--border-color)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
          ...
          {/* Data source indicator — no scenario selector (all data from API) */}
          {hasActiveInvestigation && activeScenario?.data_source && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              <span style={{ fontSize: '0.72rem', fontWeight: 600, color: 'var(--text-muted)' }}>
                Sumber Data: <strong style={{ color: '#10b981' }}>{activeScenario.data_source === 'paysim_dataset' ? 'PaySim Dataset' : 'Live Transaction Logs'}</strong>
              </span>
            </div>
          )}
        </div>
                  ...
                  onClick={() => {
                    if (onCreateCase) {
                      onCreateCase({
                        ...
                        edge: selectedEdge,
                        graphSnapshot: { nodes: filteredNodes, edges: filteredEdges }
                      });
