...
|---|---|---|---|
| 1 | **Integrasi Langsung CMS ↔ GNN (Quick-Action)** | `PageViews.jsx` (AlertsView), `App.jsx` | 🟢 **100% Live** — Tombol pintas **`🧠 Telusuri Subgraf GNNExplainer (Quick-Action)`** langsung di kartu & detail tiket alert untuk membuka graf relasi 3-hop tanpa berpindah alur secara manual. |
| 2 | **Kontrol Mode Demo Terisolasi** | `Header.jsx` | 🟢 **100% Live** — Tombol simulasi dibungkus dalam lencana **`[🧪 SANDBOX DEMO ENVIRONMENT]`** terisolasi dari *traffic production* agar tidak mengganggu transaksi *real-time*. |
| 3 | **Segregation of Duties (SoD) 3 Role RBAC** | `AuthContext.jsx`, `Sidebar.jsx`, `PageViews.jsx` | 🟢 **100% Live** — Penerapan ketat 3 Peran: **Analis AML** (Read + Triage Level 1, SoD locked pada kalibrasi), **Pejabat Kepatuhan / MLRO** (Level 3 Full Approval & submit SAR), **Pengawas Regulasi OJK/BI** (Supervisory Read-Only Audit). |
| 4 | **Arsitektur XGNN-FDS Hibrida** | `crypto-sentinel-api/app/main.py`, `GNNVisualization.jsx` | 🟢 **100% Live** — Menyajikan endpoint `GET /api/v1/sentinel/gnn/neighborhood/{account_id}` (3-hop neighborhood subgraphs, PageRank, in/out-degree, edge timestamps). |
| 5 | **GNNExplainer Subgraf Penjelas (XAI)** | `dashboard-crypto-sentinel/src/components/GNNVisualization.jsx` | 🟢 **100% Live** — Optimasi matematis Mutual Information ($\max_{G_s,F} MI(Y, (G_s,F))$) dengan penyorotan subgraf minimal $G_s$ emas menyala dan *opacity reduction* node non-kritis ke 12-30%. |
| 6 | **Interactive Temporal Timeline Slider** | `GNNVisualization.jsx` | 🟢 **100% Live** — Rekonstruksi kronologi mutasi perbankan multi-hop (`0: Semua` $\rightarrow$ `1: 09:01 Fan-Out` $\rightarrow$ `2: 09:07 Transit Layering` $\rightarrow$ `3: 09:16 Crypto Outflow`). |
| 7 | **Enterprise Case Management (CMS)** | `PageViews.jsx` (AlertsView) | 🟢 **100% Live** — Alur triage (`Unassigned` $\rightarrow$ `In Review` $\rightarrow$ `Escalated`), catatan investigasi forensik, tombol **`📄 Terbitkan Draf LTKM PPATK (goAML)`**, resolusi tiket sinkron ke API backend, dan modal preview dokumen cetak PDF. |
| 8 | **100% Dynamic Aggregation Dashboard** | `StatsGrid`, `RiskDistribution`, `BankDistribution`, `HourlyActivity`, `BlockedPatterns`, `TransactionChart` | 🟢 **100% Live** — Mengeliminasi seluruh mock statis. Seluruh metrik dikalkulasi dinamis dari stream array transaksi database aktif Neon PostgreSQL Cloud. |
| 9 | **SHAP Explainability Visualizer** | `ShapExplanation.jsx`, `TransactionTable.jsx` | 🟢 **100% Live** — Visualisasi bar chart kontribusi 29 fitur Random Forest (merah = pemicu fraud, hijau = peringan) pada setiap modal detail transaksi. |
| 10 | **Privasi & Data Masking (UU PDP No. 27/2022)** | `Header.jsx`, `TransactionTable.jsx` | 🟢 **100% Live** — Toggle penyamaran data otomatis (`maskName`, `maskAccount`, `maskNIK`) untuk kepatuhan perlindungan data pribadi perbankan. |
| 11 | **Kalibrasi FDS & Risk Appetite (POJK No. 8/2023)** | `PageViews.jsx` (RulesView) | 🟢 **100% Live** — Konfigurasi ambang batas Circuit Breaker 85% dan limit harian RBA bank internal berstandar Maker-Checker Dual-Control dengan *Immutable Audit Trail Log* (Read-Only untuk Pengawas OJK). |
