...
|---|---|---|---|
| 1 | **Pruning Menu Total Per Peran (Zero Inactive/Locked Menus)** | `Sidebar.jsx`, `AuthContext.jsx`, `App.jsx` | 🟢 **100% Live** — Setiap peran hanya menampilkan menu yang relevan. Menu di luar peran dihilangkan total: Analis (3 menu), Pejabat Kepatuhan (6 menu), Pengawas Regulasi OJK (4 menu). |
| 2 | **Filter Tenant (Bank Kuningan / Bank BJB / Apex View)** | `PageViews.jsx` (MonitoringView) | 🟢 **100% Live** — Analis AML dapat beralih cepat memantau transaksi *Bank Kuningan saja*, *Bank BJB saja*, atau *Apex View gabungan*. |
| 3 | **APOLO OJK Compliance Preview & XML Exporter** | `PageViews.jsx` (ApoloGovernanceView) | 🟢 **100% Live** — Modul formulir pelaporan resmi APOLO OJK untuk pengawas regulasi lengkap dengan ekspor berkas XML terstandarisasi. |
| 4 | **Pembersihan Identitas Persona Simulasi Perbankan** | `mockData.js`, `api.js` | 🟢 **100% Live** — Seluruh nama personil internal telah diganti menjadi entitas nasabah perbankan simulasi yang realistis (*Ahmad Faisal*, *Budi Santoso*, *Hendro Wijaya*, *Siti Rahmawati*). |
| 5 | **Integrasi Langsung CMS ↔ GNN (Quick-Action)** | `PageViews.jsx` (AlertsView), `App.jsx` | 🟢 **100% Live** — Tombol pintas **`🧠 Telusuri Subgraf GNNExplainer (Quick-Action)`** langsung di kartu & detail tiket alert untuk membuka graf relasi 3-hop tanpa berpindah alur secara manual. |
| 6 | **Kontrol Mode Demo Terisolasi** | `Header.jsx` | 🟢 **100% Live** — Tombol simulasi dibungkus dalam lencana **`[🧪 SANDBOX DEMO ENVIRONMENT]`** terisolasi dari *traffic production* agar tidak mengganggu transaksi *real-time*. |
| 7 | **Arsitektur XGNN-FDS Hibrida** | `crypto-sentinel-api/app/main.py`, `GNNVisualization.jsx` | 🟢 **100% Live** — Menyajikan endpoint `GET /api/v1/sentinel/gnn/neighborhood/{account_id}` (3-hop neighborhood subgraphs, PageRank, in/out-degree, edge timestamps). |
| 8 | **GNNExplainer Subgraf Penjelas (XAI)** | `dashboard-crypto-sentinel/src/components/GNNVisualization.jsx` | 🟢 **100% Live** — Optimasi matematis Mutual Information ($\max_{G_s,F} MI(Y, (G_s,F))$) dengan penyorotan subgraf minimal $G_s$ emas menyala dan *opacity reduction* node non-kritis ke 12-30%. |
| 9 | **Interactive Temporal Timeline Slider** | `GNNVisualization.jsx` | 🟢 **100% Live** — Rekonstruksi kronologi mutasi perbankan multi-hop (`0: Semua` $\rightarrow$ `1: 09:01 Fan-Out` $\rightarrow$ `2: 09:07 Transit Layering` $\rightarrow$ `3: 09:16 Crypto Outflow`). |
| 10 | **Enterprise Case Management (CMS)** | `PageViews.jsx` (AlertsView) | 🟢 **100% Live** — Alur triage (`Unassigned` $\rightarrow$ `In Review` $\rightarrow$ `Escalated`), catatan investigasi forensik, tombol **`📄 Terbitkan Draf LTKM PPATK (goAML)`**, resolusi tiket sinkron ke API backend, dan modal preview dokumen cetak PDF. |
| 11 | **Kalibrasi FDS & Risk Appetite (POJK No. 8/2023)** | `PageViews.jsx` (RulesView) | 🟢 **100% Live** — Konfigurasi ambang batas Circuit Breaker 85% dan limit harian RBA bank internal berstandar Maker-Checker Dual-Control dengan *Immutable Audit Trail Log*. |
